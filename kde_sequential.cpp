...
void kde_sequential(const std::vector<double>& image, std::vector<double>& density, int width, int height) {
...
int main() {
    ...
    kde_sequential(image, density, WIDTH, HEIGHT);
    double end = omp_get_wtime();

    std::cout << "Tempo (Sequencial): " << (end - start) << " segundos\n";
    return 0;
}
